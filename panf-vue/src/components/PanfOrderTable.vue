<template>
  <b-container>
    <h4>Bestellungen</h4>
    <b-table show-empty responsive striped hover :items="showOrderList" :empty-text="noOrdersText">
      ...
      <template slot="meal" slot-scope="row">{{ row.value }}</template>
      ...
      <template slot="extras" slot-scope="row">
        <span v-bind:key="idx" v-for="(extra, idx) in row.item.extras">
          <b-badge v-if="extra.type === 'remove'" variant="danger">{{ extra.name }}</b-badge>

          <b-badge v-else variant="success">+{{ extra.name }} ({{ formatPrice(extra.price) }}€)</b-badge>&nbsp;
        </span>
      </template>
